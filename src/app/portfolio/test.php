...
function sort_method($a,$b){
  return ($s["order_field"] <=$b["order_field"]) ? -1 : 1;
}
...
add_action( 'rest_api_init', function () {
  ...
  $galleries_sorted = makeGalleriesArray();

    register_rest_route( 'gallery_thumbnail/v1', '/(?P<index>[0-9-])', array(
      'methods' => 'GET',
  ...
  function getThumbnails($index){
      ...
      //  		 'url'=> get_the_post_thumbnail_url($post->ID,"portfolio-thumb"),
			//    'names' => get_post_meta($post->ID,'names',true),
			// ];
