...
export class CommissionComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   this.commission = this.route.snapshot.data['commission'];
    this.srcUrls =this.commission.srcUrls;
    this.trustedGrid = this.sanitizer.bypassSecurityTrustHtml(this.commission.grid);
  }
  ...
  ngAfterViewInit() {
   ...
   this.commissionWrapper = document.querySelector("#galleryWrapper");
   ...
   this.commissionWrapper.onscroll = () => {
    ...
    //   this.picsArray[0].style.overflow="visible";
  ...
  picZoom(photo) {
    ...
    this.renderedGrid.style.transform += `translateY(${diffY +this.commissionWrapper.scrollTop}px)`;
  ...
  browseLeft(e) {
    if (this.lightboxFlag) {
      ...
      if (this.picPointer > 0) {
        ...
        let scrollAmount = this.cumulativeOffset(photo, 5).top + photo.clientHeight / 2 -this.commissionWrapper.clientHeight / 2;;
       this.commissionWrapper.scrollTo(0, scrollAmount);
        this.picZoom(photo);
